    ...
    <div class="toolbar" id="kt_toolbar">
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
            <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                ...
                <h1 class="d-flex text-dark fw-bolder fs-3 align-items-center my-1">Listado de Egresos</h1>
            </div>
                  ...
                  {{-- <div class="row mx-5">
                    ...
                    @can('agregar proveedores') --}}
                        ...
                        <a onclick="modalCreateIngreso()" data-bs-toggle="modal" data-bs-target="#modalCreateIngreso" class="btn btn-primary btn-sm">
                            <i class="fa-solid fa-plus"></i> Registrar Egresos
                        </a>
                    {{-- @endcan --}}
                </div>
            </div>
...
@section('modals')
    @include('admin.expense.modal.create')
    @include('admin.expense.modal.edit')
@endsection
    ...
    <script>
      let income_id = 0;
        $(document).ready(function(){
            ...
            $('#expenseFormEdit').submit(function(e) {
                e.preventDefault();
                let form = document.getElementById('expenseFormEdit');
                if (!form.checkValidity()) {
                    form.reportValidity();
                    return;
                }
                let formData = new FormData(form);
                $.ajax({
                    url: "{{ url('admin/expenses/update') }}/" + income_id,
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
        ...
        function modalCreateIngreso(){
            $('#modalCreateEgresos').modal('show');
        }
        function editExpenses(id, descripcion, monto) {
            income_id =  id;
            $('#expenseFormEdit input[name="monto"]').val(monto);
            $('#expenseFormEdit textarea[name="descripcion"').val(descripcion);
            $('#modalEditEgresos').modal('show');
        }
        function deleteExpense(id, descripcion) {
            confirmar(function() {
                $.ajax({
                    url: "{{ url('admin/expenses/destroy') }}/" + id, // URL a la que se enviarán los datos
                    type: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
                    },
            ...
            }, ``, `Estas seguro de Eliminar el egresos:<br> ${descripcion} ?`);
        }
    </script>
@endpush